...
class DataProto:
    ...
    # @staticmethod
    # def custom_concat_histories(val):
    #     """Safely concatenate arrays of histories into 1D object array."""
    #     merged = []
    #     for arr in val:
    #         # arr might be 1D or 2D
    #         if arr.ndim == 1:
    ...
    @staticmethod
    def custom_concat_histories(val):
        """Plain concatenation of conversation histories arrays.
        
        Assumes all elements are JSON strings in object arrays.
        Preserves all elements including empty ones to maintain length consistency.
        """
        merged = []
        for arr in val:
            # Plain concatenation - extend with all elements from each array
            merged.extend(arr.tolist())
    ...
    def concat(data: List["DataProto"]) -> "DataProto":
        ...
        try:
            ...
            for b in batch_lst:
                print(b)
            raise RuntimeError("Failed to concatenate batch list") from e

        non_tensor_batch = list_of_dict_to_dict_of_list(list_of_dict=[d.non_tensor_batch for d in data])
        for key, val in non_tensor_batch.items():
            if key == "conversation_histories":
                # val is actually a list of array: val = [np.array(["list1", "list2"]), np.array(["list3", "list4"])]
                non_tensor_batch[key] = DataProto.custom_concat_histories(val)
            else:
                non_tensor_batch[key] = np.concatenate(val, axis=0)
