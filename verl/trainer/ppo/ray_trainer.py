...
class RayPPOTrainer:
    ...
    def _validate(self):
        ...
        for test_data in self.val_dataloader:
            ...
            unique_input_texts = []
            # Patch to make all the inputs unique
            for input_text in input_texts:
                input_text = input_text.strip() + str(unique_count)
                unique_count += 1
                unique_input_texts.append(input_text)

            sample_inputs.extend(unique_input_texts)
        ...
        metric_dict = {}
        print(" -=-=-=-=-=-=-=-=-=-=-=-= Reward Extra Infos Dict (Validation) -=-=-=-=-=-=-=-=-=-=-=-=- ")
        keys_to_pop = []
        avg_core_metric = 0
        for k, v in reward_extra_infos_dict.items():
            if k.startswith("customized_"):
                ...
                avg_value = np.mean(v)
                metric_dict.update({"customized_statistics_validation/" + k.split("customized_")[-1]: avg_value})
                print(f"{k}: {avg_value}")
                if k == "customized_cost_reward":
                    avg_core_metric = avg_value
            elif k.startswith("failure_"):
                keys_to_pop.append(k)
                value = np.mean(v)
                metric_dict.update({"failure_statistics_validation/" + k.split("failure_")[-1]: value})
                print(f"{k}: {value}")
        for k in keys_to_pop:
            reward_extra_infos_dict.pop(k)

        # check if the reward_extra_infos_dict is empty
        if not reward_extra_infos_dict:
            print("Warning: reward_extra_infos_dict is empty")
            return metric_dict
        ...
        # core_metrics = [metric_dict[pfx] for pfx in metric_dict.keys() if pfx.startswith("val/test_score/")]
    ...
    def _save_checkpoint(self, valid_save_best=False, valid_best_score=-1):
        ...
        # list all the folders under self.config.trainer.default_local_dir
        all_saving_folders = os.listdir(self.config.trainer.default_local_dir)
        for folder in all_saving_folders:
            # if it is a file, continue
            if not os.path.isdir(os.path.join(self.config.trainer.default_local_dir, folder)):
                continue
            if folder.startswith("best_") and not valid_save_best:
                continue
            if folder.startswith("global_step_") and valid_save_best:
                continue
            # remove all the folders that are not current folder
            if folder.startswith("global_step_") and not valid_save_best and folder != f"global_step_{self.global_steps}":
                shutil.rmtree(os.path.join(self.config.trainer.default_local_dir, folder))
            if folder.startswith("best_") and valid_save_best and folder != f"best_global_step_{self.global_steps}":
                shutil.rmtree(os.path.join(self.config.trainer.default_local_dir, folder))
                ...
                with _timer("step", timing_raw):
                    ...
                    with _timer("gen", timing_raw):
                        if not self.async_rollout_mode:
                            ...
                            self.async_rollout_manager.sleep()
                        # NOTE: added by Reasoning360. TODO: fix this metric later.
                    ...
                    if self.config.algorithm.adv_estimator == AdvantageEstimator.REMAX:
                        # NOTE: this is likely an abandoned branch (the async rollout not supported. not carefully maintained in Reasoning360 as well)
