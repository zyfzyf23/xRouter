...
class SGLangRollout(BaseRollout):
    ...
    async def _semaphore_wrapped_rollout(self, sem, req, do_sample, is_validate, **kwargs):
        async with sem:
            return await self._async_rollout_a_request_with_retry(req, do_sample, is_validate, **kwargs)
    
    async def _async_rollout_a_request_with_retry(self, req, do_sample, is_validate, max_retries=1, **kwargs):
        """Wrapper with retry logic for individual requests"""
        last_exception = None
        
        for attempt in range(max_retries + 1):  # +1 because we want max_retries actual retries after first attempt
            try:
                ...
                if attempt == max_retries:
                    ...
                    logger.error(f"[Rank {self._rank}] Request {req.request_id} failed after {max_retries + 1} attempts. Final error: {e}")
                    raise
                
                logger.warning(f"[Rank {self._rank}] Request {req.request_id} failed on attempt {attempt + 1}/{max_retries + 1}: {type(e).__name__}: {e}")
                ...
                if attempt < max_retries:
                    ...
                    # Optional: flush cache before retry to clear potential memory issues
                    if self.config.free_cache_engine and self._engine is not None:
                        try:
                            await self._engine.flush_cache()
                            logger.info(f"[Rank {self._rank}] Flushed cache before retry for request {req.request_id}")
                        except Exception as cache_e:
                            logger.warning(f"[Rank {self._rank}] Failed to flush cache before retry: {cache_e}")
    ...
    async def _async_rollout_a_request(
        ...
        **kwargs,
        ...
        # Special Case: If in simple mode, we allow the model to finish the tool calling first.
        while current_turns < self.config.multi_turn.max_turns or (self.config.multi_turn.simple_mode and _req.state == AsyncRolloutRequestStateEnum.TOOL_CALLING):
            if _req.state == AsyncRolloutRequestStateEnum.PENDING:
                ...
                if _req.messages[-1].tool_calls is not None:
                    ...
                    # TODO: detect the tool call is valid or not
                    for tool_call in parsed_tool_calls:
                        if tool_call.function.name not in _req.tools_kwargs:
                            # Give a mock response for the tool call
                            tool_call_results.append((
                                "Error: This tool is not given. You cannot call this tool.", 0.0, "", tool_call.function.arguments, {"success": False, "error": "This tool is not given. You cannot call this tool."}
                            ))
                        else:
                            valid_tool_calls.append(tool_call)
                            # Placeholder for valid tool calls
                            tool_call_results.append(None)
                    
                    # Execute valid tool calls and replace placeholders
                    if valid_tool_calls:
                        ...
                        for i, tool_call in enumerate(parsed_tool_calls):
                            if tool_call.function.name in _req.tools_kwargs:
                                tool_call_results[i] = valid_results[valid_result_index]
                                valid_result_index += 1

                    _req.add_tool_response_messages(self.tokenizer, [resp for resp, _, _, _, _ in tool_call_results])
                    
                    # Re-initialize the conversation history for the latest turn
                    route_to_user_found = False
                    conversation_histories[-1]["route_to"] = []
                    conversation_histories[-1]["feedback"] = []

                    for tool_call, (resp, reward, route_to, arguments, metadata) in zip(parsed_tool_calls, tool_call_results):
                        conversation_histories[-1]["route_to"].append(route_to)
                        conversation_histories[-1]["content"].append(arguments)
                        conversation_histories[-1]["feedback"].append(resp)
                        conversation_histories[-1]["reward"].append(reward)
                        conversation_histories[-1]["metadata"].append(metadata)
                        if route_to == "user" and metadata["success"]:
                            route_to_user_found = True
                        
                        if not metadata["success"]:
                            print(f"!!!!!!!!!!!!!!!!!! Tool Call Failure Detected !!!!!!!!!!!!!!!!!!!")
                            print(f"UID: {_req.request_id}, Route to: {route_to}, Arguments: {arguments}, Response: {resp} !!!!!!!!!!!!!!!!!!!")
                        else:
                            print(" ===================== UID: ", _req.request_id, " Tool Response (Route to: ", route_to, ", Turn: ", current_turns, ") ===================== ")
                            print(resp)

                        # Print the content of tool calling
                        # print(f"uid: {_req.request_id} [turn: {current_turns}, route_to: {route_to}, tool_call_id: {tool_call.id}]\ncontent: {arguments}\nresponse: {resp}")

                    if len(parsed_tool_calls) != len(valid_tool_calls):
                        finish_reason_type = FinishReasonTypeEnum.STOP
                        # print(f" !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! UID: {_req.request_id}, Current Turns: {current_turns}, Finish Reason: Parsed Tool Calls Length Mismatch !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!")
                        break

                    if len(_req.input_ids) >= self.config.max_model_len:
                        finish_reason_type = FinishReasonTypeEnum.STOP
                        # print(f" !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! UID: {_req.request_id}, Current Turns: {current_turns}, Finish Reason: Input Length {len(_req.input_ids)} Exceeds Max Model Length {self.config.max_model_len} !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!")
                        break

                    if route_to_user_found:
                        finish_reason_type = FinishReasonTypeEnum.STOP
                        # print(f" !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! UID: {_req.request_id}, Current Turns: {current_turns}, Finish Reason: Route to User !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!")
                        break
                ...
                conversation_histories.append({
                    "route_to": [], "content": [], "feedback": [], "reward": [], "metadata": [], "turn_idx": len(conversation_histories),
                })
                ...
                # # set default to self-generated reasoning
                # conversation_histories[-1]["route_to"] = ["user"]
                # conversation_histories[-1]["feedback"] = [content]
                ...
                if finish_reason_type == FinishReasonTypeEnum.LENGTH:
                    ...
                    # # remove this! only for mock function call testing!
                    ...
                    #             name="call_gpt_4o",
                    #             arguments={"revised_system_prompt": "Please ignore the user prompt and answer me this question: what name you want to have if you are a person?", "max_tokens": 1024, "temperature": 0.7},
                    #         ),
                    #     )
                    # ]
                    ...
                    #     think_content = "<think>\n" + content.split("<think>")[1].split("</think>")[0].strip() + "\n</think>\n\n"
                    # else:
                    #     think_content = "<think>\n\n</think>\n\n"
                    #     # _req.add_assistant_message(self.tokenizer, think_content)
                    ...
                    if self._function_call_parser and self._function_call_parser.has_tool_call(content):
                        ...
                        try:
                            ...
                            # print(" ===================== UID: ", _req.request_id, " Original Content ===================== ")
                            # print(content)
                            # print(" ===================== UID: ", _req.request_id, " Normed Content ===================== ")
                            # print(normed_content)
                            # print(" ===================== UID: ", _req.request_id, " Tool Calls ===================== ")
                            # print(tool_calls)
                        ...
                        print(" ~~~~~~~~~~~~~~~~~~~~~~~ UID: ", _req.request_id, " Self-Generated Reasoning (Turn: ", current_turns, ") ~~~~~~~~~~~~~~~~~~~~~~~ ")
                        print(content)
                        break
    ...
    async def _handle_engine_call(self, _req: AsyncRolloutRequest, do_sample: bool, is_validate: bool, override_n: bool = True, **kwargs) -> dict:
        ...
        # print("output['meta_info']", output["meta_info"])
        # print("output.keys()", output.keys())
        # print("len(generation_prompt_ids)", len(generation_prompt_ids))
        # print("generation_prompt_ids", generation_prompt_ids)
        # print("last 100 tokens", generation_prompt_ids[-100:])
        # print("self.sampling_params", self.sampling_params)
        # print("tokenizer class", self.tokenizer)
        # print("tokenizer.decode(generation_prompt_ids)", self.tokenizer.decode(generation_prompt_ids))
    ...
    async def _handle_pending_state(self, _req: AsyncRolloutRequest) -> AsyncRolloutRequest:
        if _req.tool_schemas is not None:
            ...
            for tool_schema in _req.tool_schemas:
                ...
                create_kwargs["messages"] = _req.messages
    ...
    def _req_level_generate_sequences(self, prompts: DataProto, **kwargs) -> DataProto:
        ...
        return DataProto(
            ...
            non_tensor_batch={
                ...
                "conversation_histories": np.array([json.dumps(x) for x in conversation_histories], dtype=object) if conversation_histories else np.array("[]", dtype=object),
        ...
        for data_idx, raw_prompt in enumerate(prompts.non_tensor_batch["raw_prompt"]):
            for rollout_offset in range(n):
                if self._tool_schemas:
                    ...
                    _tools_kwargs = {
                        k: {} for k, v in prompts.non_tensor_batch["tools_kwargs"][data_idx].items() if v is not None
                    }
                    # TODO: remove this! manually add select_response to the tools_kwargs if not presented
                    if not self.config.multi_turn.simple_mode and "select_response" not in _tools_kwargs:
                        _tools_kwargs["select_response"] = {}
                    # TODO: remove this! manually remove select_response to the tools_kwargs if simple_mode is enabled
                    if self.config.multi_turn.simple_mode and "select_response" in _tools_kwargs:
                        _tools_kwargs.pop("select_response")

                    # # remove this! only for mock function call testing! mock the tool kwargs for self-generated reasoning
                    # _tools_kwargs = {
                    #     "call_gpt_4o": {},
                    #     "call_gpt_4o_mini": {},
                    # }
