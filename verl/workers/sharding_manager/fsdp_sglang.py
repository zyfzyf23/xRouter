...
def _run_async_method(coro):
    """Helper function to run async methods in sync context"""
    try:
        # Try to get existing event loop
        loop = asyncio.get_event_loop()
        if loop.is_running():
            # If loop is running, we need to create a new thread or use run_coroutine_threadsafe
            # For simplicity, we'll use a new event loop in this case
            import threading
...
class FSDPSGLangShardingManager(BaseShardingManager):
    ...
    def update_weights(self, params):
        if self.device_mesh["infer_tp"].get_local_rank() == 0:
            ...
            _run_async_method(self.inference_engine.resume_memory_occupation())
        ...
        for tensor_index, (name, tensor) in enumerate(named_tensors):
            ...
            if self.device_mesh["infer_tp"].get_local_rank() == 0:
                # Properly await the async method
                _run_async_method(self.inference_engine.update_weights_from_tensor(
                    named_tensors=[
                        (
                            name,
    ...
    def release_memory(self):
        if self.device_mesh["infer_tp"].get_local_rank() == 0:
            # Properly await the async method
            _run_async_method(self.inference_engine.release_memory_occupation())
