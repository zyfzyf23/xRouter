...
# ------------------ prompt template ------------------ #
SYSTEM_MSG = "You are a knowledgeable assistant. Answer the following questions and think step by step. Please output the final answer within \\boxed{}. "


def make_prompt(question: str):
    return [{"role": "user", "content": SYSTEM_MSG + question.strip()}]
...
def keep_example(example, max_answer_len):
    if len(example.get("answer", "")) == 1 and example.get("answer", "").isalpha():
        return False
    return example.get("category") != "Mathematics" and example.get("difficulty") in ["University", "PhD"] and example.get("answer_type") not in ["Boolean", "Multiple Choice"] and len(example.get("answer", "").split()) > 0
...
def main():
    ...
    print(f"User‐prompt token stats — min:{arr.min()}, max:{arr.max()}, mean:{arr.mean():.1f}, std:{arr.std():.1f}")
