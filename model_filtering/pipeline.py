...
class DifficultyFilterPipeline:
    def __init__(self, args, dataset):
        ...
        # Check if using API model from router_utils
        self.use_api_model = args.model_path in MODEL_SPECS
        if self.use_api_model:
            console.print(f"🌐 Using API model: {args.model_path}")
            self.router_system = LLMRouter()
        else:
            console.print(f"🖥️  Using local model: {args.model_path}")
    ...
    def initialize_components(self):
        if self.use_api_model:
            # For API models, we don't need vLLM components
            console.print(f"🌐 Using API model: [highlight]{self.args.model_path}[/highlight]")
            
            # Get model spec for tokenizer info
            if self.args.model_path in MODEL_SPECS:
                spec = MODEL_SPECS[self.args.model_path]
                console.print(f"📋 Model: {spec.name} ({spec.provider})")
                console.print(f"💰 Cost: ${spec.input_price_per_million}/M input, ${spec.output_price_per_million}/M output")
                console.print(f"📏 Context: {spec.context_window} tokens")
            
            # Use a simple tokenizer for token estimation (could be improved)
            try:
                # Try to load a tokenizer for estimation, fallback to character-based estimation
                from transformers import AutoTokenizer
                self.tokenizer = AutoTokenizer.from_pretrained("gpt2")  # Fallback tokenizer for estimation
                console.print("🔤 Using GPT-2 tokenizer for token estimation")
            except:
                self.tokenizer = None
                console.print("⚠️ Could not load tokenizer, using character-based estimation")
                
            # Set up sampling parameters for API calls
            ...
            # Original vLLM initialization for local models
    ...
    def process_batch_outputs(self, outputs, batch_dict):
        ...
        if self.use_api_model:
            tokenizer = self.tokenizer  # Fallback tokenizer or None
        else:
        ...
        for i in range(len(outputs)):
            ...
            # Calculate token length with fallback
                ...
                if full_responses:
                    if tokenizer is not None:
                        try:
                            ...
                            # Fallback to character-based estimation
                            resp_token_counts = [len(resp) // 4 for resp in full_responses]
                    else:
    ...
    # ------------- progress ------------------------------------------------- #
    async def _generate_with_api(self, prompts):
        """Generate responses using API models via router system"""
        import asyncio
        ...
        @dataclass
        class APIOutput:
            """Mock vLLM RequestOutput structure for API responses"""
            class OutputData:
                def __init__(self, text: str):
                    self.text = text
            
            def __init__(self, outputs: List[str]):
                self.outputs = [self.OutputData(text) for text in outputs]
        
        console.print(f"🌐 Generating {len(prompts)} prompts via API...")
        
        api_outputs = []
        for prompt_messages in prompts:
            try:
                # Convert to messages format expected by router
                messages = prompt_messages if isinstance(prompt_messages, list) else [{"role": "user", "content": str(prompt_messages)}]
                
                # Generate multiple responses if n > 1
                responses = []
                for _ in range(self.sampling_params.get("n", 1)):
                    response, metadata = await self.router_system.acall_model(
                        self.args.model_path,
                        messages,
                        self.sampling_params
                    )
                ...
                api_outputs.append(APIOutput(responses))
                
            except Exception as e:
                console.print(f"[red]API error: {e}[/red]")
                # Return empty responses to match expected count
                empty_responses = [""] * self.sampling_params.get("n", 1)
                api_outputs.append(APIOutput(empty_responses))
    ...
    def _run_inference_sync(self):
        ...
        for idx, batch_dict in enumerate(batch_iter, start=start_batch_idx):
            ...
            # This is now in the sync version - will be duplicated in async
            outputs = self.model.chat(
                    batch_dict["prompt"],
                    sampling_params=self.sampling_params
            )
    ...
    async def _run_inference_async(self):
        ...
        # Get the rank output directory
        model_name = self.args.model_path.split("/")[-1] if "/" in self.args.model_path else self.args.model_path
        dataset_name = os.path.basename(self.args.dataset_parquet_path).rsplit(".parquet", 1)[0]
        rank_output_dir = self.get_rank_output_dir()
        ...
        progress_bar = tqdm(
            total=len(dataloader),
            initial=start_batch_idx,
            desc=f"🔄 DP{self.args.dp_rank} Processing (API)",
            position=0,
        )
        ...
        for idx, batch_dict in enumerate(batch_iter, start=start_batch_idx):
            ...
            # Use router system for API calls
            outputs = await self._generate_with_api(batch_dict["prompt"])
        ...
        console.print()
        console.print(Panel(f"[bold][DP-{self.args.dp_rank}] API Inference completed!", title="🏁 Finished", border_style="green"))
        console.print(f"⏱️  [DP-{self.args.dp_rank}] Total time: [time]{self.format_time(elapsed_total)}[/time]")
        console.print(
            f"ℹ️ [DP-{self.args.dp_rank}] All per-batch JSON files are ready. "
            "Run the separate reward script to score and assemble final results."
        ...
        if self.router_system:
            ...
            for model_id, stats in self.router_system.usage_stats.items():
                if stats.total_calls > 0:
                    console.print(f"  {model_id}: {stats.total_calls} calls, ${stats.total_cost:.4f} total cost")
            
        console.print(f"🏁 [DP-{self.args.dp_rank}] API Process complete!")